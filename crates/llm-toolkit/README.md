...
| **Persistent Context Management** | Attach context information that remains visible throughout long conversations without being buried in history. PersonaAgent strategically places context based on conversation length. | `PayloadContent::Context`, `with_context()`, `ContextConfig`, `.with_context_config()` | Implemented |
