## Agent API and Multi-Agent Orchestration


`llm-toolkit` provides a powerful agent framework for building multi-agent LLM systems with a clear separation of concerns.

## Contents

- [Agent API: Capability and Intent Separation](./01-agent-api-capability-and-intent-separati.md)
- [Multi-Modal Agent Communication with Payload](./02-multi-modal-agent-communication-with-pay.md)
- [Multi-Agent Orchestration](./03-multi-agent-orchestration/00-index.md)
- [Context-Aware Detection & Enrichment](./04-context-aware-detection-enrichment.md)
- [High-Performance Parallel Execution with ParallelOrchestrator](./05-high-performance-parallel-execution-with.md)
- [Parallel Orchestrator with Human-in-the-Loop (HIL)](./06-parallel-orchestrator-with-human-in-the-.md)
