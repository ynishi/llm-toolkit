...
| **Direct API Clients** | HTTP API clients for LLM providers without CLI dependency. Includes retry support, per-request timeout control (`with_timeout_secs`), multi-modal payloads, and provider-specific features (Gemini thinking, Google Search). | `AnthropicApiAgent`, `GeminiApiAgent`, `OpenAIApiAgent` | Implemented (v0.58.0) |
