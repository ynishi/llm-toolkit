//! Integration tests for llm-toolkit-expertise

use llm_toolkit_expertise::{
    Anchor, ContextMatcher, ContextProfile, Expertise, KnowledgeFragment, Priority, TaskHealth,
    WeightedFragment,
};
...
fn test_context_filtering_combinations() {
    ...
    let prompt2 = expertise.to_prompt_with_context(&ContextMatcher::new().with_task_type("Debug"));
    ...
    let prompt4 = expertise
        .to_prompt_with_context(&ContextMatcher::new().with_task_health(TaskHealth::AtRisk));
...
fn test_task_health_behaviors() {
    ...
    let on_track_prompt = expertise
        .to_prompt_with_context(&ContextMatcher::new().with_task_health(TaskHealth::OnTrack));
    ...
    let off_track_prompt = expertise
        .to_prompt_with_context(&ContextMatcher::new().with_task_health(TaskHealth::OffTrack));
